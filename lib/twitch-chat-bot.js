...
const TwitchChatBotErrorHandler = require("../modules/error-handler/error-handler.js");
...
/* initialization */
	...
	onClientConnect(address, port)
	{
		...
		this.#connected = true;
		this.log({
			msg: "Twitch chat client connected (tmi.js)",
			args: arguments,
		});
...
/* event management */
	...
	fireEvent(event, a)
	{
		...
		this.#eventHandlers[event].sort((a, b) => a.priority - b.priority);
...
/* globals */
	...
	getGlobal(k)
	{
		if (typeof k !== "string" ||
			k.length === 0) {
			this.errorHandler.warn("ERROR_GLOBALS_GET_INVALID_KEY", k || typeof k);
		}

		if (this.#globalKeys.includes(k.toLowerCase()) === false) {
			this.errorHandler.warn("ERROR_GLOBALS_GET_UNKNOWN_KEY", k || typeof k);
		}
