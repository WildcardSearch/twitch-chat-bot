...
/* initialization */
	...
	onClientConnect(address, port)
	{
		...
		this.#connected = true;
		this.log( { msg: "Twitch chat client connected (tmi.js)", args: arguments } );
...
/* internal events */
	...
	ready()
	{
		...
		if (typeof this.options.permissionsOverride === "object" &&
			Object.keys(this.options.permissionsOverride).length > 0) {
			for (let [k, p] of Object.entries(this.options.permissionsOverride)) {
				if (typeof k === "string") {
					k = k.replace(/^!/, "").trim();
				} else {
					continue;
...
/* globals */
	...
	registerGlobal(g)
	{
		...
		for (const globalVar of g) {
			...
			if (typeof handler !== "function") {
				this.log(`bad global handler "${handler.name || "non-function"}"`);
